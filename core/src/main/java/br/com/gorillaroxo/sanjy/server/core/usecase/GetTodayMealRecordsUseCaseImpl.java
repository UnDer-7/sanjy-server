...
class GetTodayMealRecordsUseCaseImpl implements GetTodayMealRecordsUseCase {
    ...
    public List<MealRecordDomain> execute(final ZoneId userTimezone) {
        ...
        final Instant startOfDay = currentLocalDate.atTime(LocalTime.MIN).atZone(userTimezone).toInstant();
        final Instant endOfDay = currentLocalDate.atTime(LocalTime.MAX).atZone(userTimezone).toInstant();
